...
try:
    ...
    print('Avantes doesnt work')

try:
    from oceanoptics.OOptSpec import OOptSpec
except :
    print('Ocean Optics doesnt work')
    
try:
    from broadcom.Qmini import Qmini
except :
    print('Qmini doesnt work')
...
class Spectrometer():
    ...
    def find(self,autoconnect=False):
        ...
        try:
            ...
            print('Avantes is not connecting')
            print(error)
            self.avantes=None
        ...
        try:
            ...
            print('Ocean Optics is not connecting')
            print(error)
            self.ocean=None
        ...
        try:
            ...
            print('Qmini Optics is not connecting')
            print(error)
            self.qmini=None
    ...
    def connect(self, DeviceN=0):
        ...
        if DeviceN < len(self.devices):
            if self.devices[DeviceN][0]=="Avantes":
                ...
                self.connected=True
                self.SN=self.spectrometer.SN
            elif self.devices[DeviceN][0]=="Qmini":
                self.spectrometer=copy.copy(self.qmini)
                self.spectrometer.connect(SN=self.devices[DeviceN][1])
                self.dev_handle=self.spectrometer.dev_handle
                self.lam=self.spectrometer.lam
                self.connected=True
    ...
    def config_measure(self,Tintegration=0.01,Naverage=1,HighRes=True,IntensityCalibration=False,ExtTrig=False):
        ...
        if self.connected:
            self.spectrometer.config_measure(Tintegration,Naverage,HighRes,IntensityCalibration,ExtTrig) 
