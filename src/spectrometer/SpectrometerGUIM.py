...
from Spectrometer_classM import Spectrometer
...
# class SpecGUI(QDialog):
    def __init__(self,show=True):
        ...
        self.ExtTrig.clicked.connect(self.setAcquisition)
        self.ICalib.clicked.connect(self.setAcquisition)
    ...
    def show_roi(self):
        ...
        if self.roi:
            self.roiMax = pg.InfiniteLine(angle=90, movable=True, pen=(0, 0, 0), pos=(self.spectrometer.lam[0].mean() + 10))
            self.roiMin = pg.InfiniteLine(angle=90, movable=True, pen=(0, 0, 0), pos=(self.spectrometer.lam[0].mean() - 10))
    ...
    def v_cursor(self):
        if self.VerCursor.isChecked():
            if not self.vLine: self.vLine = pg.InfiniteLine(angle=90, movable=True, pen=(0,0,0), pos=self.spectrometer.lam[0].mean())
    ...
    def connect(self):
        """open connection interface"""
        try:
            index = self.connectwindow.table.selectedIndexes()
            index = [i.row() for i in index]
    ...
    def save(self, file=None):
        ...
        if file != '':
            Txtime=str(time.asctime(time.gmtime(time.time())))
            for i in range(len(self.spec)):
                spec=self.spec[i]
                lam=self.spectrometer.lam[i]
                if len(spec) > 2:
                    X=lam
                    if self.SubstractBkg:
                        Y=spec-self.bkg[i]
                        ...
                        Y=spec
                    file=file.split('.spec')[0]
                    np.savetxt(file+'_'+str(i)+'.spec', np.concatenate((np.array(X).reshape((-1, 1)),
                                                            np.array(Y).reshape((-1, 1))), axis=1),
                               header='# wavelength nm\t intensity a.u. ' +
                               '<>\t' + str(self.spectrometer.config_parameters[i]['vendor'])+'<>'+
                               ' SN'+str(self.spectrometer.config_parameters[i]['SN'])+ '\t' +'<>'+
    ...
    def addshowstatus(self, text):
        Ctext=self.status.toPlainText()
        self.status.setPlainText(Ctext + text)
    ...
    def setAcquisition(self):
        ...
        self.Naverage=Nav
        self.ExternalTrig=self.ExtTrig.isChecked()
        self.Icalib=self.ICalib.isChecked()
        self.spectrometer.config_measure(Tintegration=Tint,Naverage=Nav,ExtTrig=self.ExternalTrig,IntensityCalibration=self.Icalib)
        
        if len(self.spectrometer.spectrometer) == 1:
            if self.ExternalTrig and self.spectrometer.spectrometer[0].Type!= 'Avantes':
                self.showstatus('external trigger works only for Avantes')
            if self.Icalib and self.spectrometer.spectrometer[0].Type!= 'Qmini':
                self.addshowstatus('\n I calibration works only for Qmini')
    ...
    def getspectrum(self):
        ...
        #     self.spectrometer.spectrometer.start_measure()
        self.spectrometer.start_measure()
        ...
        while (not self.spectrometer.isdataready()) and self.sampling:
            time.sleep(self.Msleeptime)
            t=(time.time()-self.Tstart)*1000
            self.WaitBar.setValue(int(t))
            self.app.processEvents()
            # print('waiting in GUI')
        if self.sampling:
            data0=self.spectrometer.read_data()
            data=[d[0] for d in data0]
            # print(data)
            self.spec=[np.array(data[i])[:len(self.spectrometer.lam[i])] for i in range(len(data))]
            # print(self.spec)
            self.ShowPlot()
    ...
    def log_spec(self):
        if self.Log_file != False and self.Log_file != '':
            ...
            if self.firstLog:
                ...
                self.Ttstart=float(time.time())
                y = np.insert(y, 0, 0)
                data = np.row_stack((ref, y))
                #fmt='%f',
                
                np.savetxt(self.Log_file+'.logspec', data, 
                ...
                # print(y)
                y = np.insert(y, 0, time.time()-self.Ttstart)
                print(self.Ttstart,time.time(),time.time()-self.Ttstart)
                with open(self.Log_file+'.logspec', "a") as f:
                    np.savetxt(f, y.reshape(1, -1),delimiter='\t', comments='')
    ...
    def ShowPlot(self):
        ...
        for i in range(len(self.spec)):
            X=self.spectrometer.lam[i]
            if self.SubstractBkg:
                Y=self.spec[i].copy()-self.bkg[i].copy()
            else:
                Y=self.spec[i].copy()
            RangeX=[min(RangeX[0],X.min()),max(RangeX[1],X.max())]
            RangeY=[min(RangeY[0],Y.min()),max(RangeY[1],Y.max())]
            self.SShowPlot(X,Y)
        ...
        if not self.AutoScaleV:
            ...
            self.SetYmin.setValue(RangeY[0])
            self.SetYmax.setValue(RangeY[1])
        
        if not self.AutoScaleH:
            plot.setXRange(self.SetXmin.value(),self.SetXmax.value())
        else:
            plot.setXRange(X.min(),X.max())
            self.SetXmin.setValue(RangeX[0])
            self.SetXmax.setValue(RangeX[1])
    
    def SShowPlot(self, X, Y):
        """show single spectrum"""
        plot = self.plot
