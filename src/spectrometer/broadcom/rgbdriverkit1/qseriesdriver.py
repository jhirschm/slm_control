...
class Qseries(calibratedspectrometer.CalibratedSpectrometer):
    ...
    def search_devices(serial_number=None):
        ...
        if serial_number is None:
            devs_gen = core.find(find_all=True, idVendor=Qseries.__ID_VENDOR)
        else:
            devs_gen = core.find(find_all=True, idVendor=Qseries.__ID_VENDOR, custom_match=find_serial_number(serial_number))
...
#class CommunicationInterface(object):
    ...
    def usbOpen(self):
        ...
        util.claim_interface(self.dev, 0) # FixMe: [Errno 16] Resource busy also indicates another instance is already running
